...
// On définit l'état initial à l'extérieur pour pouvoir le réutiliser facilement
const initialState = {
...
const DailyForm = ({ onEntryCreated, editData, onCancelEdit }) => {
  ...
  // Synchronise le formulaire quand on clique sur "Modifier" (editData)
  useEffect(() => {
    if (editData) {
      setFormData({
        ...initialState, // On garde la structure de base
        ...editData,     // On écrase avec les données reçues
        // On s'assure que les sous-objets existent pour éviter les crashs
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Notifier le parent (App.jsx) pour rafraîchir la liste et les stats
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {editData && (
        <div className="flex justify-between items-center bg-brand/10 border border-brand/20 p-4 rounded-2xl animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-black uppercase tracking-widest text-brand">Modification en cours</span>
          </div>
          <button 
            type="button"
      ...
      {/* SECTION MOOD */}
      <section className="bg-surface p-6 rounded-3xl border border-white/5 shadow-xl">
          ...
          {[
            ...
            <button
              ...
              className={`flex flex-col items-center gap-2 p-4 rounded-2xl border transition-all duration-300 ${
                ...
                : 'bg-bg-dark border-transparent opacity-30 hover:opacity-50'
              }`}
            >
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        >
          <div className="flex gap-3">
            <Input 
              ...
              value={formData.sport?.duration || ''}
              onChange={(e) => setFormData({ ...formData, sport: { ...formData.sport, duration: parseInt(e.target.value) || 0 } })}
            />
            <Input 
              type="text" placeholder="Type (Yoga, Run...)" 
              value={formData.sport?.activityType || ''}
        ...
        >
          <Input 
            ...
            onChange={(e) => setFormData({ ...formData, reading: { ...formData.reading, duration: parseInt(e.target.value) || 0 } })}
          />
        </ControlCard>

        <div className={`p-5 rounded-3xl border flex justify-between items-center transition-all duration-300 ${formData.alcohol ? 'bg-red-500/10 border-red-500/30' : 'bg-surface border-white/5'}`}>
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-xl transition-colors ${formData.alcohol ? 'bg-red-500 text-white' : 'bg-white/5 text-white/40'}`}>
              <Wine size={20} />
            </div>
            <span className={`font-bold transition-colors ${formData.alcohol ? 'text-white' : 'text-white/40'}`}>Alcool</span>
          </div>
          <input 
            type="checkbox" 
      ...
      <Button type="submit" loading={loading} className="w-full py-5 text-lg shadow-2xl">
        <Send size={20} className="mr-2" /> 
        {editData ? 'Mettre à jour l\'entrée' : 'Enregistrer ma journée'}
      </Button>
    </form>
  );
};
